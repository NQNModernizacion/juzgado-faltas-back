...
class AdminController extends Controller
{
    ...
    public function getPersonInfoByDni(Request $request, string $dni)
    {
        try {
            ...
            if (!$dniClean || strlen($dniClean) < 7 || strlen($dniClean) > 10) {
                return sendResponse(null, 'DNI no válido', 422);
            }

            $persona = PersonasAdmin::query()->where('documento', $dniClean)->first();

            if (!$persona) {
                return sendResponse(null, 'No se encontró persona con ese DNI', 404);
            }
    ...
    public function syncRoles(Request $request, User $user)
    {
        try {
            ...
            if (is_array($roleIds)) {
                ...
                return sendResponse(null, 'Enviar role_ids[] o roles[]', 422);
            }

            if (!count($names)) {
                return sendResponse(null, 'Roles inválidos', 422);
            }
    ...
    public function syncPermissions(Request $request, User $user)
    {
        try {
            ...
            if (!is_array($permIds)) {
                return sendResponse(null, 'permission_ids debe ser un array', 422);
            }
    ...
    public function userByDni(Request $request, string $dni)
    {
        try {
            ...
            if (!$persona) {
                return sendResponse(null, 'No se encontró persona', 404);
            }
            ...
            if (!$userId) {
                return sendResponse([
                    ...
                    'user' => null,
                ], 'La persona no tiene usuario en users', 200);
            }
            ...
            if (!$user) {
                return sendResponse([
                    ...
                    'user' => null,
                ], 'ReferenciaID no existe en users', 409);
            }
